<script lang="ts">
	...
	let selectedChord: typeof selectedChordDefault | keyof typeof chords = selectedChordDefault; // Default selected chord
	...
	function playChord(chord: keyof typeof chords) {
		tones.forEach(stopTone);
		tones = [...tones, ...chords[chord].map((frequency) => createTone({ frequency }))];
	...
	function drawWaveform() {
		...
		tones.forEach((tone, index) => {
			...
			if (tone.pan >= 0) {
				drawChannel(canvasContext, sliceWidth, tone, chartHeight, chartHeight, index);
			}
		});
...
<div class="audioController">
	...
	<div class="grid">
		{#if tones.length > 0}
			...
			{#each tones as t, i}
				...
				{#if numberInputMode}
					...
					<input type="number" bind:value={t.volume} min={0} max={1} step="0.01" />
					<input type="number" bind:value={t.pan} min={-1} max={1} step="0.01" />
				{:else}
		...
		<button
			on:click={() =>
				selectedChord === selectedChordDefault ? addTone() : playChord(selectedChord)}
		>
			➕
		</button>
